...
using AutoFixture;
using AutoMapper;
using Defra.Trade.Catch.Certificate.Internal.V3INTERNAL.ApiClient.Api;
using Defra.Trade.Catch.Certificate.Internal.V3INTERNAL.ApiClient.Client;
...
using Defra.Trade.Events.Services.CatchCertificates.Logic.V3.MessageProcessors;
using Microsoft.Extensions.Logging;
using Moq;
using Shouldly;
using Xunit;
using ApiModel = Defra.Trade.Catch.Certificate.Internal.V3INTERNAL.ApiClient.Model;

namespace Defra.Trade.Events.Services.CatchCertificates.Logic.Tests.V3.MessageProcessors;
...
public class ProcessingStatementMessageProcessorTests
{
    ...
    public async Task ProcessAsync_FailedApiPost_ThrowsException()
    {
        ...
        _apiClient
            .Setup(c =>
                c.CreateProcessingStatementWithHttpInfoAsync(
                    "v3-internal",
    ...
    public async Task ProcessAsync_SuccessfulApiPost_Success_NoVersion()
    {
        ...
        _apiClient
            .Setup(c =>
                c.CreateProcessingStatementWithHttpInfoAsync(
                    "v3-internal",
                    It.Is<ApiModel.ProcessingStatement>(d => d.DocumentNumber == inboundModel.DocumentNumber && d._Version == 3),
                    It.IsAny<int>(),
                    It.IsAny<CancellationToken>()))
            .ReturnsAsync(new ApiResponse<object>(HttpStatusCode.NoContent, null));
